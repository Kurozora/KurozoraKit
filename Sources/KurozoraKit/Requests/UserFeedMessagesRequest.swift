...
/// A request that fetches feed messages for a given user.
public struct UserFeedMessagesRequest: Sendable {
	// MARK: - Properties
	...
	public func response() async throws -> ResourceCollection<FeedMessage> {
		let request = KKRequest<ResourceCollection<FeedMessage>>(
			path: self._cursor?.urlString ?? KKEndpoint.Users.feedMessages(self.userIdentity).endpointValue,
			method: .get,
