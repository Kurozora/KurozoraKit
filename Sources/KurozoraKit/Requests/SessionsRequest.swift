...
/// A request that fetches the authenticated user's sessions.
public struct SessionsRequest: Sendable {
	...
	public func response() async throws -> ResourceCollection<SessionIdentity> {
		let request = KKRequest<ResourceCollection<SessionIdentity>>(
			path: self._cursor?.urlString ?? KKEndpoint.Me.Sessions.index.endpointValue,
			method: .get,
