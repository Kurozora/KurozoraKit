...
/// A request that fetches the authenticated user's favorites.
public struct MyFavoritesRequest: Sendable {
	// MARK: - Properties
	...
	public func response() async throws -> FavoriteLibraryResponse {
		...
		let request = KKRequest<FavoriteLibraryResponse>(
			path: self._cursor?.urlString ?? KKEndpoint.Me.Favorites.index.endpointValue,
			method: .get,
