...
/// A request that fetches the authenticated user's reminders.
public struct MyRemindersRequest: Sendable {
	// MARK: - Properties
	...
	// MARK: - Initializers
	internal init(context: RequestContext, kind: LibraryKind) {
	...
	public func response() async throws -> ReminderLibraryResponse {
		...
		let request = KKRequest<ReminderLibraryResponse>(
			path: self._cursor?.urlString ?? KKEndpoint.Me.Reminders.index.endpointValue,
			method: .get,
