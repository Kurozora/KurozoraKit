//
//  FeedMessageRepliesRequest.swift
//  KurozoraKit
//
...
/// A request that fetches the replies for a given feed message.
public struct FeedMessageRepliesRequest: Sendable {
	// MARK: - Properties
	...
	public func response() async throws -> ResourceCollection<FeedMessage> {
		let request = KKRequest<ResourceCollection<FeedMessage>>(
			path: self._cursor?.urlString ?? KKEndpoint.Feed.Messages.replies(self.messageIdentity).endpointValue,
			method: .get,
