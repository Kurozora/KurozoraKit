//
//  FeedMessageQuotesRequest.swift
//  KurozoraKit
//
...
/// A request that fetches the quote re-shares for a given feed message.
public struct FeedMessageQuotesRequest: Sendable {
	// MARK: - Properties
	...
	/// Sets the sort order for the returned quotes.
	...
	public func response() async throws -> ResourceCollection<FeedMessage> {
		...
		let request = KKRequest<ResourceCollection<FeedMessage>>(
			path: self._cursor?.urlString ?? KKEndpoint.Feed.Messages.quotes(self.messageIdentity).endpointValue,
			method: .get,
