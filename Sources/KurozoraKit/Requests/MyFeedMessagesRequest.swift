...
/// A request that fetches the authenticated user's feed messages.
public struct MyFeedMessagesRequest: Sendable {
	// MARK: - Properties
	...
	public func response() async throws -> ResourceCollection<FeedMessage> {
		let request = KKRequest<ResourceCollection<FeedMessage>>(
			path: self._cursor?.urlString ?? KKEndpoint.Me.Feed.messages.endpointValue,
			method: .get,
