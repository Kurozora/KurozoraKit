...
/// A request that fetches the list of blocked users for a given user.
public struct UserBlockListRequest: Sendable {
	// MARK: - Properties
	...
	public func response() async throws -> ResourceCollection<UserIdentity> {
		let request = KKRequest<ResourceCollection<UserIdentity>>(
			path: self._cursor?.urlString ?? KKEndpoint.Users.blocking(self.userIdentity).endpointValue,
			method: .get,
